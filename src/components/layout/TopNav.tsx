              ...
              return (
                <div key={g.group} className="mt-4 first:mt-0">
                  ...
                  {/* เทคนิค grid-template-rows: 0fr -> 1fr ให้ความสูงเลื่อนลื่นโดยไม่ต้องคำนวณความสูงจริงด้วย JS —
                      ลิงก์ยังอยู่ใน DOM ตลอดเวลา จึงต้องซ่อนด้วย visibility เพิ่มด้วย ไม่ใช่แค่ height:0 เฉยๆ */}
                  <div
                    className={`grid transition-[grid-template-rows] duration-200 ease-out ${isOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
                  >
                    <div className={`overflow-hidden ${isOpen ? "visible" : "invisible"}`}>
                      <div className="flex flex-col gap-1 pt-0.5">{g.links.map(renderLink)}</div>
                    </div>
                  </div>
                </div>
              );
